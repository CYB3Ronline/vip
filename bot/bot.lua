...
SUDO_ID = {431115872,460070882}
Full_Sudo = {431115872}
ChannelLogs = -1001050478885
BotHelper = 90
Channel = '@Hosien890'
MsgTime = os.time() - 60
Plan1 = 2592000
...
sendText(msg.chat_id, msg.id,d.." روز تا پایان شارژ گروه باقی مانده است",  'md' )
end
end
...
sendText(msg.chat_id, msg.id, "اوه شت :(\nمن نمیتوانم یک فرد دارای مقام را محدود کنم ", 'md')
else
...
sendText(msg.chat_id, msg.id, '• زمان پاکسازی خودکار تنظیم شد به :  *'..num..'* ثانیه', 'md')
end
end
...
text= "کاربر " .. (Company.sender_user_id or '0000Null0000') .. " شما یک اخطار دریافت کردید\nلطفا تو گروه بی احترامی نکنید قوانین رو رعایت کنید تا اخطار نگیرید\nتعداد اخطار های شما : " ..warnhash .. " عدد\nاگر به " .. warn .. " برسد اخراج میشوید"
...
sendText(msg.chat_id, msg.id,  '• Your Name : ['..CompanyName..']\n• User ID : ['..msg.sender_user_id..']\n• Rank : ['..rank..']\n• Total Msgs : ['..Msgs..']\n','md')
end
GetUser(msg.sender_user_id,GetName)
end
...
elseif is_Owner(msg) then
text =[[•راهنمای کار با ربات برای صاحب گروه

⬅️فعال کردن خوش امد گو
welcome enable

⬅️غیرفعال کردن خوش امد گو
welcome disable
...
⬅️خالی کردن لیست افراد سکوت شده
clean mutelist

⬅️ترفیع دادن کاربر به مقام کمک مدیر
promote
...
⬅️سکوت کردن تمام کاربران 
mute all

⬅️رفع سکوت تمام کاربران
unmute all

⬅️فعال کردن عضو اجباری
forceadd enable

⬅️غیرفعال کردن عضو اجباری
forceadd disable

⬅️تنظیم حالت عضو اجباری
setforce [new user] or [all user]

⬅️تنظیم حداکثر عضو اجباری
setforcemax [num] 

⬅️تنظیم زمان برای پاک کردن پیام های خود ربات (درصورت فعال بودن عضو اجباری)
settimedelete [num]

⬅️تنظیم حالت سکوت
setmute all [deletemsg] or [restricted]

⬅️تنظیم حالت قفل خودکار
automute all [timestart-timestop]

⬅️فعال سازی قفل خودکار
automuteall enable

⬅️غیرفعال سازی قفل خودکار
automuteall disable

⬅️ویژه کردن کاربر
setvip

⬅️حذف کاربر از لیست ویژه
remvip 

⬅️نمایش اعضای ویژه
viplist 

⬅️پاکسازی لیست اعضای ویژه
clean viplist 

⬅️اخراج تمامی ربات ها 
clean bots

⬅️فیلتر کردن کلمه
filter ...

⬅️حذف کلمه از لیست فیلتر 
unfilter ...

⬅️اخراج کاربر از گروه
kick

⬅️مسدود کردن کاربر از گروه
ban

⬅️لیست کاربران مسدود شده 
banlist 

⬅️خالی کردن لیست مسدود
clean banlist

⬅️تنظیم پیام رگباری
setflood [num]

⬅️تنظیم زمان پیام رگباری
setfloodtime [num]

⬅️تنظیم حالت برخورد با پیام رگباری
setflood [kickuser] or [muteuser]

⬅️پاکسازی تمامی پیام های گروه
clean msgs 

⬅️تنظیم لینک گروه 
setlink [link] or [reply]

⬅️ارتقا مقام تمامی ادمین ها 
config 
...
⬅️حذف کاربران محدود شده از لیست
clean restricts
...
⬅️بیصدا کردن
mute photo/music/voice/document/video/game/location/contact/text
مثال : mute photo

⬅️لغو بیصدا
unmute photo/music/voice/document/video/game/location/contact/text
مثال : unmute photo

⬅️تنظیم مقدار اخطار
warnmax [num]

⬅️اخطار دادن به کاربر 
warn
...
⬅️پاکسازی تمام پیام های یک کاربر 
delall [reply]
...
elseif is_Mod(msg) then
text =[[•راهنمای کار با ربات برای کمک مدیر گروه
...
elseif not is_Mod(msg) then
text =[[شما کاربر عادی هستید تنها میتوانید از دستورات زیر استفاده کنید
